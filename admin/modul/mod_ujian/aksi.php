...
if (isset($_GET['act'])) {
    switch ($_GET['act']) {
        case 'add':
            ...
            $menit = $_POST['menit'];
            $detik = $_POST['detik'];
            $kd_soal = isset($_POST['kd_soal']) ? $_POST['kd_soal'] : '';
            $kd_kelas = isset($_POST['kd_kls']) ? $_POST['kd_kls'] : '';
            $kd_mapel = $_POST['mapel'];
            $kd_guru = $_POST['kd_guru'];
            $tglup = date("Y-m-d H:i:s");

            if (is_array($kd_kelas) && !empty($kd_kelas)) {
                foreach ($kd_kelas as $kd) {
                    ...
                    if ($qins) {
                        $qt = "INSERT INTO timeline (jenis,id_jenis,waktu,kd_kelas,kd_mapel,kd_guru) VALUES ('ujian','$kd_ujian','$tglup','$kd','$kd_mapel','$kd_guru')";
                ...
                // Handle jika $kd_kelas bukan array atau kosong
                echo "<script>alert('Gagal'); location='../../media.php?module=ujian'</script>";
            }
